...
// --- REFINED: loadState and initializeTasks ---
...
// --- (Director Communication and Worker Assignment functions remain the same) ---
    ...
    ws.on('message', message => {
        ...
        switch (data.type) {
            ...
            case 'startComputation':
                ...
                initializeTasks(true); // Pass true to clear state file
                fs.writeFileSync(TASK_LOG_FILE, 'Timestamp,TaskID,WorkerID,Status,Duration(ms)\n');
                fs.writeFileSync(WORKER_LOG_FILE, 'Timestamp,WorkerID,IPAddress,CPU_Cores,Browser,Status\n');
                workers.forEach(w => {
            ...
            // --- (Other cases like 'registerWorker', 'result', etc. remain the same) ---
            case 'registerWorker':
